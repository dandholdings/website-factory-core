...
# --- JSON parsing ---------------------------------------------------------
def parse_json_strict_or_extract(raw: str, context: str = "", max_retries: int = 3) -> dict:
    """
    Parse a JSON object from model output with strict validation, error logging, and retry logic.
    
    Args:
        raw: Raw text from LLM response
        context: Context for error logging (e.g., "hub_fill", "page_generation")
        max_retries: Maximum number of retry attempts for JSON repair
    ...
    if isinstance(raw, (dict, list)):
        try:
            ...
            logger.warning(f"Failed to convert raw object to dict: {e}")
            raw = json.dumps(raw)

    raw = str(raw).replace("\ufeff", "").replace("\x00", "").strip()

    if not raw:
        error_msg = f"Empty model output for context: {context}"
        logger.error(error_msg)
        raise json.JSONDecodeError(error_msg, raw, 0)
    ...
    def _log_json_error(attempt: int, error: str, raw_snippet: str = ""):
        """Log JSON parsing errors with context."""
        snippet_preview = raw_snippet[:200] + "..." if len(raw_snippet) > 200 else raw_snippet
        logger.error(
            f"JSON parse error (attempt {attempt}/{max_retries}) for {context}: {error}\n"
            f"Raw snippet: {snippet_preview}"
        )

    def _attempt_json_parse(candidate: str, attempt: int = 1) -> Optional[dict]:
        """Attempt to parse JSON with various strategies."""
        strategies = [
            ("direct_parse", lambda: json.loads(candidate)),
            ("balanced_extract", lambda: _extract_balanced_json(candidate)),
            ("repair_missing_braces", lambda: _repair_missing_braces(candidate)),
            ("repair_trailing_commas", lambda: _repair_trailing_commas(candidate)),
            ("extract_with_regex", lambda: _extract_json_with_regex(candidate)),
        ]
        ...
        for strategy_name, parse_func in strategies:
            try:
                ...
                if isinstance(obj, dict):
                    logger.debug(f"JSON parsed successfully with strategy '{strategy_name}' (attempt {attempt})")
                    return obj
                if isinstance(obj, list) and len(obj) == 1 and isinstance(obj[0], dict):
                    logger.debug(f"JSON array with single dict parsed with strategy '{strategy_name}' (attempt {attempt})")
                    return obj[0]
                if isinstance(obj, list):
                    logger.debug(f"JSON array wrapped in dict with strategy '{strategy_name}' (attempt {attempt})")
                    return {"data": obj}
            except Exception as e:
    ...
    def _extract_balanced_json(s: str) -> dict:
        ...
        if end is None:
            raise json.JSONDecodeError(f"JSON {open_char}{close_char} appears truncated", s, 0)

        snippet = _strip_fences(s[start:end])
        return json.loads(snippet)

    def _repair_missing_braces(s: str) -> dict:
        """Repair JSON with missing closing braces/brackets."""
    ...
    def _repair_trailing_commas(s: str) -> dict:
    ...
    def _extract_json_with_regex(s: str) -> dict:
        """Extract JSON using regex patterns."""
        # Try to find JSON object
        json_obj_pattern = r'\{[^{}]*\}(?:\s*,\s*\{[^{}]*\})*'
        match = re.search(json_obj_pattern, s, re.DOTALL)
        if match:
            try:
        ...
        match = re.search(json_arr_pattern, s, re.DOTALL)
        if match:
            try:
                return json.loads(match.group(0))
            except:
        ...
        raise json.JSONDecodeError("No JSON found with regex patterns", s, 0)

    # Main parsing loop with retries
    cand = _strip_fences(raw)
    last_error = None
    
    for attempt in range(1, max_retries + 1):
        try:
            obj = _attempt_json_parse(cand, attempt)
            if obj:
                ...
                # Log success
                logger.info(f"JSON parsed successfully for {context} (attempt {attempt})")
                return obj
        except Exception as e:
            last_error = e
            _log_json_error(attempt, str(e), cand[:500])
            
            # On last attempt, try one more aggressive repair
            if attempt == max_retries:
                logger.warning(f"All JSON parsing attempts failed for {context}, trying aggressive repair")
                try:
                    # Try to extract any JSON-like structure
                    json_pattern = r'(\{.*\}|\[.*\])'
                    matches = re.findall(json_pattern, cand, re.DOTALL)
                    if matches:
                        for match in matches:
                            try:
                                repaired = json.loads(match)
                                if isinstance(repaired, dict):
                                    logger.warning(f"Recovered JSON from aggressive extraction for {context}")
                                    return repaired
                            except:
                                continue
                except Exception as final_error:
                    logger.error(f"Final JSON parse failure for {context}: {final_error}")
    
    # All attempts failed
    error_msg = f"Failed to parse JSON after {max_retries} attempts for context: {context}"
    logger.error(error_msg)
    raise json.JSONDecodeError(error_msg, raw, 0)
